...
class PurchaseDashboard(models.AbstractModel):
    ...
    def get_dashboard_data(self, company_id=None, company_ids=None, year=None, month=None, date_from=None, date_to=None, category_ids=None, partner_ids=None, exclude_partner_ids=None, exclude_branches=False):
        ...
        if company_ids and len(company_ids) > 0:
            ...
            if not exclude_branches:
                ...
                for cid in list(resolved_ids):
                    children = self.env['res.company'].search([('parent_id', '=', cid)])
            ...
            if not exclude_branches:
                child_companies = self.env['res.company'].search([('parent_id', '=', company_id)])
        ...
        company_partners = self.env['res.company'].search([]).mapped('partner_id').ids
        branch_partners = []
        if 'res.branch' in self.env:
            branch_partners = self.env['res.branch'].search([]).mapped('partner_id').ids
    ...
    def _get_spend_analysis(self, company_ids, year=None, month=None, category_ids=None, exclude_partner_ids=None):
        ...
        if month and month != 'all':
            ...
            cr.execute(f"""
                SELECT EXTRACT(DAY FROM am.invoice_date)::integer AS day_num,
                       SUM(aml.price_total) AS spend_val
                FROM account_move_line aml
                JOIN account_move am ON am.id = aml.move_id
                LEFT JOIN product_product pp ON pp.id = aml.product_id
                LEFT JOIN product_template pt ON pt.id = pp.product_tmpl_id
                WHERE am.company_id IN %s AND am.move_type = 'in_invoice' AND am.state = 'posted'
                  AND aml.display_type = 'product'
                  AND EXTRACT(YEAR FROM am.invoice_date) = %s
                  AND EXTRACT(MONTH FROM am.invoice_date) = %s
                  AND {cat_clause} {exclude_am}
                GROUP BY day_num ORDER BY day_num ASC
            """, [company_ids, year_val, month_num] + cat_params + exclude_am_params)
            rows = cr.fetchall()
            day_spend_map = {d: 0.0 for d in range(1, 32)}
            for day_num, val in rows:
                if day_num in day_spend_map:
                    day_spend_map[day_num] = float(val)
            chart_data = [{'month': str(d), 'spend': day_spend_map[d], 'saving': day_spend_map[d] * 0.08} for d in range(1, 32)]
            ytd_spend = sum(day_spend_map.values())
            monthly_spend = ytd_spend
        else:
            cr.execute(f"""
                SELECT TO_CHAR(am.invoice_date, 'Mon') AS month_name,
                       EXTRACT(MONTH FROM am.invoice_date) AS month_num,
                       SUM(aml.price_total) AS spend_val
                FROM account_move_line aml
                JOIN account_move am ON am.id = aml.move_id
                LEFT JOIN product_product pp ON pp.id = aml.product_id
                ...
                WHERE am.company_id IN %s AND am.move_type = 'in_invoice' AND am.state = 'posted'
                  ...
                  AND EXTRACT(YEAR FROM am.invoice_date) = %s
                  AND {cat_clause} {exclude_am}
                GROUP BY month_name, month_num ORDER BY month_num ASC
            """, [company_ids, year_val] + cat_params + exclude_am_params)
            rows = cr.fetchall()
            spend_map = {m: 0.0 for m in ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']}
            for name, num, val in rows:
                if name in spend_map:
                    spend_map[name] = float(val)
            chart_data = [{'month': m, 'spend': spend_map[m], 'saving': spend_map[m] * 0.08} for m in spend_map]
    ...
    def get_price_tendency(self, company_id=None, granularity='month', year=None, date_from=None, date_to=None,
                           product_ids=None, category_ids=None, product_name=None, exclude_branches=False):
        ...
        if exclude_branches:
            if not self.env['res.company'].browse(company_id).parent_id:
                ho = self.env['res.company'].search([('parent_id', '=', company_id), ('name', 'ilike', 'Head Office')], limit=1)
                if ho:
                    company_ids.append(ho.id)
        else:
            child_companies = self.env['res.company'].search([('parent_id', '=', company_id)])
            company_ids.extend(child_companies.ids)
        company_ids_tup = tuple(company_ids)
    ...
    @api.model
    def get_purchasable_products(self, company_id=None, search='', exclude_branches=False):
