...
export class PurchaseDashboard extends Component {
            ...
            this.spendChart = new window.Chart(spendCtx, {
                ...
                options: {
                    ...
                    onClick: (evt, activeElements) => {
                        ...
                        // Build domain and open posted vendor bills for only this period
                        const domain = [
                            ['move_type', '=', 'in_invoice'],
                            ['state', '=', 'posted'],
    ...
    openModelList(resModel, domain = [], options = {}) {
        ...
        if (this.state.data.company_ids && this.state.data.company_ids.length > 0) {
            if (['purchase.order', 'purchase.order.line', 'account.move', 'stock.picking', 'employee.purchase.requisition'].includes(resModel)) {
        ...
        if (this.state.filters.exclude_branches === 'without' && this.state.data.branch_partner_ids) {
            if (['purchase.order', 'purchase.order.line', 'account.move', 'stock.picking'].includes(resModel)) {
